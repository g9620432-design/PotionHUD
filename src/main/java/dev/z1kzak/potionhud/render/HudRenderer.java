...
public final class HudRenderer {
    ...
    public static void renderHud(GuiGraphics g, DeltaTracker delta) {
        ...
        // the settings and position screens draw their own live copy
        if (mc.screen instanceof dev.z1kzak.potionhud.screen.PositionScreen
                || mc.screen instanceof dev.z1kzak.potionhud.screen.ConfigScreen) {
            return;
        }
    ...
    private static List<EffectEntry> collect(HudConfig cfg, Minecraft mc) {
        ...
        for (MobEffectInstance inst : raw) {
            ...
            Identifier id = iconIdOf(holder);
            String key = id == null ? holder.toString() : id.toString();
    ...
    private static Identifier iconIdOf(Holder<MobEffect> holder) {
        return holder.unwrapKey().map(k -> {
            Identifier id = k.identifier();
            return Identifier.fromNamespaceAndPath(id.getNamespace(), "mob_effect/" + id.getPath());
        }).orElse(null);
    }
    ...
    private static EffectEntry fake(String name, Holder<MobEffect> holder, int amp, int ticks,
        ...
        return new EffectEntry(name, amp, ticks, false, false, beneficial, harmful, color,
                iconIdOf(holder), holder, "preview:" + name, 1f, 0f);
    }
}
