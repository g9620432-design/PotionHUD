 ...
 *
 * Version agnostic — the per-version module wraps this in a real {@code Screen} and forwards
 * input, because Mojang changed the input signatures during 1.21.x.
 */
public class ConfigMenu {
    ...
    // ── rendering ────────────────────────────────────────────────────────────
    /** Dim plus the live HUD preview, drawn underneath the window. */
    ...
    public boolean drag(double mx, double my) {
        if (picker != null && picker.drag(mx, my)) {
            return true;
        }
        return active != null && active.drag(mx, my);
    }
        ...
        if (in(mx, my, contentX, contentY, contentW, contentH)) {
            for (Option o : visibleOptions()) {
                if (o.isOver(mx, my) && (o instanceof SliderOption || o instanceof CycleOption)
                        && Platform.get().shiftDown()) {
                    return o.scroll(amount);
                }
            }
            scroll = Math.max(0, Math.min(maxScroll, scroll - amount * 18));
            return true;
        }
        return false;
    }
