...
/**
 * Minecraft 1.21.9 – 1.21.10 implementation of the compat seam: sprite lookup, the GUI matrix stack,
...
public final class PlatformImpl implements Platform {
    ...
    public void blitEffectIcon(GuiGraphics g, Holder<MobEffect> holder, int x, int y, int size,
                               float alpha) {
        ResourceLocation sprite = net.minecraft.client.gui.Gui.getMobEffectSprite(holder);
        if (sprite == null) {
            return;
        }
    ...
    private void allocate(int w, int h) {
        ...
        CaptureTexture wrapper = new CaptureTexture(glassTexture, glassView);
        glassW = w;
        glassH = h;
    ...
    private static final class CaptureTexture extends AbstractTexture {
        CaptureTexture(GpuTexture t, com.mojang.blaze3d.textures.GpuTextureView v) {
            this.texture = t;
            this.textureView = v;
            setClamp(true);
